"""
Second-half model (LOCKED): Decoder + Head
Input:  (2, 2, 64)  — bottleneck feature map produced by the first-half model
Output: (4,)         — class probabilities

First run  → builds model with random weights, saves weights + input to
             output_dir, then proceeds.
Later runs → loads the saved weights + input so results are identical every time.
"""

import os
import shutil
import time
from contextlib import contextmanager
from pathlib import Path

import numpy as np
from keras.src.layers import GlobalAveragePooling2D, UpSampling2D
from tensorflow.keras.layers import Conv2D, Dense, Input
from tensorflow.keras.models import Model

import hls4ml

# ---------------------------------------------------------------------------
# Output directory + lock files (weights/input live here; only wiped on first run)
# ---------------------------------------------------------------------------
output_dir = str(Path(__file__).parent / 'hls4ml_output' / 'hls4mlprj_second_half_vitis_unified')
WEIGHTS_FILE = Path(output_dir) / 'weights.h5'
INPUT_FILE = Path(output_dir) / 'x_input_locked.npy'

FIRST_RUN = not (WEIGHTS_FILE.exists() and INPUT_FILE.exists())

if FIRST_RUN:
    if os.path.exists(output_dir):
        shutil.rmtree(output_dir)
    os.makedirs(output_dir)
    print('*** FIRST RUN — weights and input will be generated and locked. ***')
else:
    print(f'*** LOCKED RUN — loading weights from {WEIGHTS_FILE} ***')
    print(f'***              loading input    from {INPUT_FILE} ***')

# ---------------------------------------------------------------------------
# Crash-safe timing logger
# ---------------------------------------------------------------------------
_TIMING_LOG = Path(output_dir) / 'timing_log.txt'
_tlog = open(_TIMING_LOG, 'a', buffering=1)  # append so prior lock info is preserved


def _tlog_write(line: str) -> None:
    """Write a line and fsync so it survives a crash."""
    _tlog.write(line + '\n')
    os.fsync(_tlog.fileno())


_tlog_write(f'=== Timing log started at {time.strftime("%Y-%m-%d %H:%M:%S")} ===')
_tlog_write(f'Log file: {_TIMING_LOG}')
_tlog_write(f'First run: {FIRST_RUN}')
_tlog_write('')


@contextmanager
def timed_step(name: str):
    """Context manager that times a block and appends the result to the log."""
    _tlog_write(f'[START] {name}  ({time.strftime("%H:%M:%S")})')
    t0 = time.perf_counter()
    try:
        yield
    finally:
        elapsed = time.perf_counter() - t0
        _tlog_write(f'[END]   {name}  elapsed={elapsed:.3f}s')
        _tlog_write('')
        print(f'[timing] {name}: {elapsed:.3f}s')


# ---------------------------------------------------------------------------
# Xilinx tool-chain paths
# ---------------------------------------------------------------------------
os.environ['XILINX_VITIS'] = '/tools/Xilinx/Vitis/2023.2'
os.environ['XILINX_VIVADO'] = '/tools/Xilinx/Vivado/2023.2'
os.environ['PATH'] = os.environ['XILINX_VITIS'] + '/bin:' + os.environ['XILINX_VIVADO'] + '/bin:' + os.environ['PATH']

# ---------------------------------------------------------------------------
# Model definition — decoder + head
# Input  (2, 2, 64)  →  Output (4,)
# ---------------------------------------------------------------------------
with timed_step('1. Keras model definition (second half)'):
    inputs = Input(shape=(2, 2, 64))

    # -------- Decoder --------
    x = UpSampling2D((2, 2))(inputs)  # 2 -> 4
    x = Conv2D(32, (3, 3), padding='same', activation='relu')(x)

    x = UpSampling2D((2, 2))(x)  # 4 -> 8
    x = Conv2D(16, (3, 3), padding='same', activation='relu')(x)

    # -------- Head --------
    x = GlobalAveragePooling2D()(x)
    x = Dense(64, activation='relu')(x)
    outputs = Dense(4, activation='softmax')(x)

    model = Model(inputs, outputs, name='second_half')
    model.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])

    if FIRST_RUN:
        model.save_weights(str(WEIGHTS_FILE))
        _tlog_write(f'Weights saved (locked) → {WEIGHTS_FILE}')
        print(f'Weights saved (locked) → {WEIGHTS_FILE}')
    else:
        model.load_weights(str(WEIGHTS_FILE))
        _tlog_write(f'Weights loaded from {WEIGHTS_FILE}')

    model.summary()

# ---------------------------------------------------------------------------
# Input data — locked after first run
# ---------------------------------------------------------------------------
BATCH_SIZE = 10

if FIRST_RUN:
    np.random.seed(42)
    X_input = np.random.rand(BATCH_SIZE, 2, 2, 64).astype(np.float32)
    np.save(str(INPUT_FILE), X_input)
    _tlog_write(f'Input saved (locked) → {INPUT_FILE}')
    print(f'Input saved (locked) → {INPUT_FILE}')
else:
    X_input = np.load(str(INPUT_FILE))
    _tlog_write(f'Input loaded from {INPUT_FILE}  shape={X_input.shape}')
    print(f'Input loaded from {INPUT_FILE}  shape={X_input.shape}')

# ---------------------------------------------------------------------------
# hls4ml config
# ---------------------------------------------------------------------------
with timed_step('2. hls4ml config'):
    config = hls4ml.utils.config_from_keras_model(model, granularity='name')
    config['Model']['Strategy'] = 'resource'
    config['Model']['ReuseFactor'] = 8
    config['Model']['Precision'] = 'ap_fixed<4,2>'

# ---------------------------------------------------------------------------
# VitisUnified conversion
# ---------------------------------------------------------------------------
with timed_step('3. convert_from_keras_model'):
    vitis_unified_model = hls4ml.converters.convert_from_keras_model(
        model,
        hls_config=config,
        output_dir=output_dir,
        backend='VitisUnified',
        io_type='io_stream',
        board='kv260',
        part='xck26-sfvc784-2LV-c',
        clock_period='10ns',
        input_type='float',
        output_type='float',
        axi_mode='axi_stream',
    )

# ---------------------------------------------------------------------------
# Model visualization
# ---------------------------------------------------------------------------
with timed_step('4. plot_model'):
    hls4ml.utils.plot_model(
        vitis_unified_model,
        to_file=os.path.join(output_dir, 'hls4ml_model.png'),
        show_shapes=True,
        show_layer_names=True,
        show_precision=True,
    )
    print(f'Model graph saved to {output_dir}/hls4ml_model.png')

# ---------------------------------------------------------------------------
# C-simulation
# ---------------------------------------------------------------------------
with timed_step('5. vitis_unified_model.compile (csim bridge)'):
    print('\n--- Compiling HLS model (csim bridge) ---')
    vitis_unified_model.compile()

with timed_step('6. vitis_unified_model.predict (csim)'):
    print('\n--- Running prediction via csim bridge ---')
    y_hls = vitis_unified_model.predict(X_input)

with timed_step('7. keras model.predict (reference)'):
    print('\n--- Keras reference prediction ---')
    y_keras = model.predict(X_input)

y_hls = y_hls.reshape(y_keras.shape)  # HLS bridge flattens outputs; restore shape
print('\nMax absolute difference (HLS vs Keras):', np.max(np.abs(y_hls - y_keras)))

# Save results
np.save(os.path.join(output_dir, 'x_input.npy'), X_input)
np.save(os.path.join(output_dir, 'y_pred_hls.npy'), y_hls)
np.save(os.path.join(output_dir, 'y_pred_keras.npy'), y_keras)
print(f'\nSaved inputs/outputs to {output_dir}')

# ---------------------------------------------------------------------------
# HLS synthesis + bitfile generation
# ---------------------------------------------------------------------------
with timed_step('8. vitis_unified_model.build (synth + bitfile)'):
    print('\n--- Building bitfile (synthesis + implementation) ---')
    vitis_unified_model.build(synth=True, bitfile=True, log_to_stdout=True)

_tlog_write(f'=== All steps complete at {time.strftime("%Y-%m-%d %H:%M:%S")} ===')
_tlog.close()
print(f'\nTiming log written to {_TIMING_LOG}')
