...
class VitisUnifiedPartial_MetaGen(VitisUnified_MetaGen):
    ...
    def get_enqueue_func_stream2rstream(self, tensorVar, idx: int):
        result = (
            "enqueue_layerStream2layer<{INPUT_LAYER_STREAM}, {INPUT_LAYER_ARR}, {SIZE}>"
            "({SRC_STREAM}, {RAW_STREAM}, {IS_LAST});"
        ).format(
    ...
    def get_dequeue_func_rstream2atom(self, tensorVar, idx: int, lastcheck: str, out_dma_type: str = "float"):
        result = (
            "dequeue_layer2atom<{ATOMIC_TYPE}, {OUTPUT_LAYER_ARR}, {SIZE}>" "({DES_STREAM}, {RAW_STREAM}, {IS_LAST_CHECK});"
        ).format(
    ...
    def get_dequeue_func_rstream2stream(self, tensorVar, idx: int, lastcheck: str):
        result = (
            "dequeue_layer2layer<{OUTPUT_LAYER_STREAM}, {OUTPUT_LAYER_ARR}, {SIZE}>"
            "({DES_STREAM}, {RAW_STREAM}, {IS_LAST_CHECK});"
        ).format(
