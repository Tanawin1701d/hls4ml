...
class VitisACIPFlowParConfig:
    def __init__(self, config, model_inputs, model_outputs):
        ...
        assert (
            len(model_inputs) >= 1
        ), "Only models with at least one input tensor are currently supported by VitisAcceleratorIPFlowPartialBackend"
        assert (
            len(model_outputs) >= 1
        ), "Only models with one output tensor are currently supported by VitisAcceleratorIPFlowPartialBackend"
