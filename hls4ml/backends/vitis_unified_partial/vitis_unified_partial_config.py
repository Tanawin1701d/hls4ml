...
class VitisUnifiedPartialConfig(VitisUnifiedConfig):
    ...
    def __init__(self, config, model_inputs, model_outputs, mgs_mng):
        ...
        self.free_interim_input = (
            self.config.get('MultiGraphConfig', {}).get('IOInterimType', {}).get("Input") == "io_free_stream"
        )
        self.free_interim_output = (
            self.config.get('MultiGraphConfig', {}).get('IOInterimType', {}).get("Output") == "io_free_stream"
        )
