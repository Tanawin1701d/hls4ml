"""
First-half model (LOCKED): Encoder + Bottleneck
Input:  (8, 8, 1)
...
First run  → builds model with random weights, saves weights + input to
             'locked/' directory next to this script, then proceeds.
...
# ---------------------------------------------------------------------------
output_dir = str(Path(__file__).parent / 'hls4ml_output' / 'hls4mlprj_first_half_locked_vitis_unified_exp')
...
if FIRST_RUN:
    # Wipe only on the first run so lock files are never accidentally deleted
...
# ---------------------------------------------------------------------------
# Model definition — architecture is always the same
